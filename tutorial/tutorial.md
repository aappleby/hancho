...
Arbitrarily-nested arrays will be flattened out and their elements stringified
...
Fields that are used globally in multiple rules can be set on ```hancho.config```, which will make them visible in ***every*** rule:
```py
