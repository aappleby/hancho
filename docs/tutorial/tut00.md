...
|task_path |No| The _absolute_ path to the directory you want to run the task in.<p>Defaults to the working directory.
|command_files|No| Files that 'command' needs in order to run, relative to task_path <p>Used to ensure that 'command' reruns when config files or scripts change.
