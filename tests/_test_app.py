...
class TestApp(unittest.TestCase):
    ...
    def init(self, *, argv):
        hancho.init_for_testing(argv = argv) # type: ignore
...
#        self.assertEqual(12, script.globals.foo)
    ...
    def test_verbosities(self):
        ...
        self.assertEqual(hancho.Log.Level.INFO, hancho.Log.log_level_out)

        self.init(argv = ["--log.level=warning"])
        self.assertEqual(hancho.Log.WARNING, hancho.Log.log_level_out)

        self.init(argv = ["--log.level=error"])
        self.assertEqual(hancho.Log.ERROR, hancho.Log.log_level_out)

        self.init(argv = ["--log.level=critical"])
        self.assertEqual(hancho.Log.CRITICAL, hancho.Log.log_level_out)

        with self.assertRaises(ValueError):
            self.init(argv = ["--log.level=boo"])
