...
class TestTasks(unittest.TestCase):
    ...
    #        t = hancho.Task(
    #            command = lambda task : force_touch(task.expanded.out_file),
    ...
    def test_task_collision(self):
        ...
        """
        hancho.Task(
            command = lambda task : (os.utime(src, None) for src in task.expanded.out_obj),
            in_src=__file__,
            out_obj="colliding_output.txt",
        )
        task2 = hancho.Task(
            command = lambda task : force_touch(task.expanded.out_obj),
            in_src=__file__,
            out_obj="colliding_output.txt",
        )
        self.run_tasks(1)
        ...
        def run():
            ...
            hancho.Task(
                command=[
                    ...
                    lambda task : force_touch(task.expanded.out_obj),
                ],
                in_src=[],
                out_obj="result.txt",
            )
    ...
    def test_input_changed(self):
        ...
        def run():
            ...
            compile = hancho.Dict(
                ...
                command = lambda task : shutil.copy(task.expanded.in_src, task.expanded.out_obj),
                in_src=None,
...
#        task = hancho.Task()
    ...
    def test_sync_command(self):
        def sync_command(task):
            force_touch(task.expanded.out_obj)
    ...
    def test_sync_callback(self):
        def sync_callback(task):
            time.sleep(0.1)
            force_touch(task.expanded.out_file)
    ...
    def test_task_creates_task(self):
        ...
        def callback(task):
            hancho.Task(command = lambda task : force_touch(task.expanded.out_obj), in_src=[], out_obj="dummy.txt")
            return []
    ...
    def test_dependency_skipped(self):
        def run():
            ...
            task1 = hancho.Task(
                name="task1",
                #command="cp {in_file} {out_file}",
                command = lambda task : shutil.copy(task.expanded.in_file, task.expanded.out_file),
